...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_long_text = bool(re.search(r'<p|<div.*class=.*text|article|<span.*text', content, re.IGNORECASE)) if not is_css_file else False
        has_form = bool(re.search(r'<form|<input|password|credit|card|payment', content, re.IGNORECASE)) if not is_css_file else False
        complex_elements = len(re.findall(r'<input|<select|<textarea|<option', content, re.IGNORECASE)) if not is_css_file else 0
